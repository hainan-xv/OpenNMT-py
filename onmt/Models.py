...
class RNNEncoder(EncoderBase):
    ...
    def __init__(self, rnn_type, bidirectional, num_layers,
        ...
        self.tdnn = TDNN([-3, 0, 3], hidden_size * 2, hidden_size * 2, full_context=True)
    ...
    def forward(self, src, lengths=None, encoder_state=None):
        ...
        [a, b, c] = memory_bank.size()
        zeros = Variable(torch.zeros([3, b, c])).cuda()

        memory_bank = torch.cat((zeros, memory_bank, zeros), 0)

#        print ("before ", memory_bank.size())
        memory_bank = self.tdnn(memory_bank)
#        print ("after ", memory_bank.size())
        return encoder_final, memory_bank
...
#        print ("x is ", x.size())
        ...
        result = torch.transpose(F.relu(conv_out), 1, 2)
